...
public submitTransfer(): void {
  ...
  this.store.submitTransferResponse$
    .pipe(
      map((response) => {
        if (!response || !response.data || !response.data.submitExternalTransfer) {
          // Handle the case where the response or expected data is null
          throw new Error('Invalid response or missing transfer data');
        }
      ...
      catchError((error) => {
        ...
        return throwError(() => new Error('Failed to submit transfer'));
      })
    )
    .subscribe({
      next: (transfers) => {
